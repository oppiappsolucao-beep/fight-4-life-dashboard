...
import { contentTypeLabel, type ModalityItem } from "../../types/modality";
...
export default function OwnerModalidadesPage() {
  ...
  const load = useCallback(() => {
    ...
    apiFetch<{ modalidades: ModalityItem[] }>("/owner/modalidades")
      .then((data) => {
        ...
        setSelectedIds(data.modalidades.filter((item) => item.active).map((item) => item.id));
      })
      .catch((err) =>
  ...
  return (
    <OwnerSectionPage
      ...
      description="Escolha quais modalidades sua unidade oferece aos alunos. Depois libere professores e vincule planos."
    >
      {loading ? (
