...
const MENU_ITEMS = [
  ...
  { to: "/dev/planos", label: "Planos", icon: PlanIcon },
    ...
    <aside className={sidebarShellClass(open)}>
      <div className="flex items-start justify-between gap-3 border-b border-slate-200 px-5 py-5">
        <div className="min-w-0">
          ...
          {user?.name && (
            <p className="mt-1 truncate text-sm font-medium text-slate-800">{user.name}</p>
          )}
          {user?.email && (
        ...
        <button
          ...
          className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-200 text-slate-600 md:hidden"
        >
          ×
        </button>
      </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition ${
                isActive
                  ? "bg-[#E8EEF4] text-[#2E496C]"
                  : "text-slate-500 hover:bg-[#D9E0E8]/70 hover:text-[#2E496C]"
              }`
            }
          >
            <Icon />
      ...
      <div className="space-y-2 border-t border-slate-200 p-4">
        <NavLink
          to="/"
          onClick={onClose}
          className="block w-full rounded-lg border border-slate-200 px-3 py-2 text-center text-xs font-medium text-slate-500 transition hover:border-[#5B7595]/40 hover:text-[#2E496C]"
        >
          Área do aluno
        </NavLink>
        <button
          ...
          className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 transition hover:border-[#5B7595]/40 hover:text-[#5B7595]"
        >
          Sair
        </button>
      </div>
